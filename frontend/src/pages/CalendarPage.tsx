...
import {
  ...
  DollarSign, CheckSquare, Handshake, Info, Zap, AlertTriangle
} from 'lucide-react';
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-[100]">
      <div className="glass-panel w-full max-w-lg max-h-[85vh] flex flex-col shadow-sm border border-border-main">
        <div className="flex justify-between items-center p-6 pb-4 flex-shrink-0 border-b border-border-main">
          <h2 className="text-2xl font-bold text-text-primary">{t('calendar.createModal.title')}</h2>
        ...
        <div className="flex space-x-4 p-6 pt-4 flex-shrink-0 border-t border-border-main">
...
const CalendarPage: React.FC = () => {
        ...
        <div className="divide-y divide-border-main">
          {filteredEvents
            ...
            .map((event) => {
              ...
              return (
                <div key={getEventId(event)} onClick={() => handleEventClick(event)} className="p-5 hover:bg-hover cursor-pointer transition-colors flex items-center justify-between group hover-lift">
                  <div className="flex items-start space-x-5">
                    ...
                    <div>
                      <div className="flex items-center gap-3">
                        <h4 className="text-base font-bold text-text-primary group-hover:text-primary-start transition-colors">{event.title}</h4>
                        {isRescheduled && (
  ...
  const renderMonthView = () => {
    ...
    const cellClass = 'min-h-[120px] border-r border-b border-border-main relative group transition-colors hover:bg-hover flex flex-col p-2';
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push(
        <div key={day} className={`${cellClass} ${today ? 'bg-primary-start/10' : ''}`} onClick={() => handleDayClick(date)}>
          <div className={`text-sm font-bold mb-2 flex justify-between items-center p-1 ${today ? 'text-primary-start' : 'text-text-muted'}`}>
            <span className={`w-8 h-8 flex items-center justify-center rounded-full ${today ? 'bg-primary-start text-white shadow-sm shadow-primary-start/40' : ''}`}>
              {day}
            </span>
          </div>
          <div className="flex-1 w-full space-y-1 overflow-visible relative">
              ...
              return (
                <div key={eventId} className="relative w-full">
                  <button
                    ...
                    className={`w-full text-left px-2 py-1.5 rounded-md border flex items-center gap-2 transition-all duration-200 shadow-sm ${style.bg} ${style.border} group-hover:shadow-md ${isHovered ? 'scale-[1.05] z-10 ring-2 ring-primary-start/50' : ''}`}
                  >
                  ...
                  <AnimatePresence>
                    {isHovered && (
                      <motion.div
                        ...
                        className="absolute left-0 top-full mt-2 z-[999] w-72 glass-panel p-4 shadow-sm border border-border-main"
                      >
            ...
            {dayEvents.length > 4 && (
              <div className="text-xs text-text-muted px-1 text-center font-bold hover:text-text-primary transition-colors cursor-pointer mt-1">
