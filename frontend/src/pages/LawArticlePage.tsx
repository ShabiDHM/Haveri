...
// PHOENIX PROTOCOL - LAW ARTICLE V8.6 (FIXED TYPES)
  ...
  return (
    <motion.div className="w-full min-h-screen pb-16 bg-canvas" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
      <div className="max-w-4xl mx-auto px-6 sm:px-8 pt-28">
        ...
        <button onClick={handleBack} className="group mb-8 flex items-center gap-3 text-text-muted hover:text-text-primary transition-colors font-bold text-sm uppercase tracking-widest hover-lift">
        ...
        <div className="glass-panel p-0 flex flex-col overflow-hidden shadow-sm border border-border-main">
          ...
          <div className="bg-surface px-8 py-6 flex justify-between items-center border-t border-border-main">
            <button onClick={handleBack} className="text-xs font-black uppercase tracking-widest text-text-muted hover:text-primary-start transition-colors flex items-center gap-2 hover-lift"><ArrowLeft size={14} /> {t('lawArticle.backToSearch', 'Kthehu te kërkimi')}</button>
