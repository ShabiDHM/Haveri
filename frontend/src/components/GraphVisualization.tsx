...
// --- UI SUB-COMPONENTS (MOVED OUTSIDE) ---
const InspectorClientDetails: React.FC<{ node: GraphNode }> = ({ node }) => (
    <div className="p-3 bg-slate-800 rounded-lg border border-slate-700">
        <span className="text-xs text-slate-400 block mb-1">Lifetime Value</span>
...
const InspectorInventoryDetails: React.FC = () => (
     <div className="p-3 bg-slate-800 rounded-lg border border-slate-700">
        <span className="text-xs text-slate-400 block mb-1">Used In</span>
        <span className="text-white font-medium">Espresso Macchiato, Cappuccino</span>
    </div>
);
const DefaultInspectorDetails: React.FC<{ node: GraphNode }> = ({ node }) => (
    <div className="p-3 bg-slate-800 rounded-lg border border-slate-700">
       ...
       <div className="flex items-center gap-2">
            <span className={`w-2 h-2 rounded-full ${node.status === 'Unpaid' || node.status === 'Overdue' ? 'bg-red-500' : 'bg-emerald-500'}`}></span>
            <span className="text-white font-medium">{node.status || 'Active'}</span>
       </div>
   </div>
);
...
const LoadingOverlay: React.FC = () => (
    <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center z-30">
        <div className="flex flex-col items-center gap-2">
            ...
            <span className="text-purple-300 font-mono text-sm">ANALYZING...</span>
        </div>
    </div>
);
...
const GraphVisualization: React.FC = () => {
  ...
  const nodeCanvasObject = useCallback((node: any, ctx: CanvasRenderingContext2D) => {
    ...
    if (isRisk) {
        ...
        ctx.shadowBlur = Math.abs(Math.sin(Date.now() / 500)) * 15;
      ...
      {selectedNode && (
        <div className="absolute right-4 top-4 bottom-4 w-72 bg-slate-900/95 backdrop-blur border-l border-slate-700 shadow-2xl p-5 flex flex-col rounded-xl animate-in slide-in-from-right-10 duration-200">
            <div className="flex justify-between items-start mb-6">
                <div>
                    <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">{t(`graph.${selectedNode.group?.toLowerCase()}`, 'Entity')}</h3>
            ...
            <div className="mt-auto space-y-3">
                ...
                {selectedNode.group === 'Client' && activeMode === 'OPPORTUNITY' && (
                     <button className="w-full flex items-center justify-center gap-2 bg-purple-600 hover:bg-purple-500 text-white py-2.5 rounded-lg transition-colors font-medium text-sm"><FileText size={16} /> Draft Sales Email</button>
                )}
                {selectedNode.group === 'Client' && activeMode !== 'OPPORTUNITY' && (
                    <button className="w-full flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-500 text-white py-2.5 rounded-lg transition-colors font-medium text-sm"><Phone size={16} /> Call Client</button>
                )}
                {(selectedNode.group === 'Invoice' && (selectedNode.status === 'Unpaid' || selectedNode.status === 'Overdue')) && (
                    <button className="w-full flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-500 text-white py-2.5 rounded-lg transition-colors font-medium text-sm"><MessageCircle size={16} /> Send WhatsApp Reminder</button>
                )}
                {(selectedNode.group === 'Inventory' && (selectedNode.status === 'Unpaid')) && ( 
                    <button className="w-full flex items-center justify-center gap-2 bg-orange-600 hover:bg-orange-500 text-white py-2.5 rounded-lg transition-colors font-medium text-sm"><ShoppingCart size={16} /> Reorder Stock</button>
                )}
                {selectedNode.group === 'Case' && (
                     <button className="w-full flex items-center justify-center gap-2 bg-slate-700 hover:bg-slate-600 text-white py-2.5 rounded-lg transition-colors font-medium text-sm"><Briefcase size={16} /> Open Case</button>
                )}
                 {selectedNode.group === 'Product' && (
                     <button className="w-full flex items-center justify-center gap-2 bg-slate-700 hover:bg-slate-600 text-white py-2.5 rounded-lg transition-colors font-medium text-sm"><Package size={16} /> View Recipe</button>
                )}
            </div>
        </div>
      )}
