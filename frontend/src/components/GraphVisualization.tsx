...
const GraphVisualization: React.FC = () => {
  ...
  // --- 2. Smart Focus Engine ---
  useEffect(() => {
    if (fgRef.current && data.nodes.length > 0) {
      setTimeout(() => {
        // INTELLIGENT ZOOM:
        // If data is small (startup phase), use a fixed elegant zoom.
        // If data is large (war room), fit everything to screen.
  ...
  const nodeCanvasObject = useCallback((node: any, ctx: CanvasRenderingContext2D) => {
    ...
    else if (node.status === 'Pending') statusColor = '#eab308';
