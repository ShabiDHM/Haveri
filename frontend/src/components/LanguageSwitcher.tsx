...
// PHOENIX PROTOCOL - CORE FUNCTIONALITY (I18N) V2.0 (DESIGN SYSTEM ALIGNMENT)
...
// 4. USAGE: To be integrated into the main application Header.tsx.
  ...
  return (
    <div className="relative">
      <button
        ...
        className="flex items-center gap-2 p-2 text-text-secondary hover:text-text-primary hover:bg-hover rounded-lg transition-colors"
      >
        <Globe size={20} />
      ...
      {isOpen && (
        <>
          ...
          <div className="absolute right-0 mt-2 w-40 bg-glass backdrop-blur-xl border border-border-main rounded-xl shadow-xl py-2 z-20 animate-in fade-in slide-in-from-top-2">
            {Object.keys(languages).map((lng) => (
              <button
                key={lng}
                onClick={() => changeLanguage(lng)}
                className="w-full flex items-center px-4 py-2 text-sm text-text-secondary hover:text-text-primary hover:bg-hover transition-colors"
              >
