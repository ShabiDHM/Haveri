...
// PHOENIX PROTOCOL - INBOX V2.1 (CLEANUP)
// 1. CLEANUP: Removed the unused 'AnimatePresence' import to resolve the warning.
// 2. STATUS: Fully optimized and warning-free.
...
export const InboxTab: React.FC = () => {
    ...
    return (
        <div className="flex flex-col lg:flex-row gap-6 h-[calc(100vh-12rem)] min-h-[600px]">
            {/* Folder Sidebar */}
            <div className={`${selectedMessage ? 'hidden lg:flex' : 'flex'} flex-col w-full lg:w-1/4 max-w-xs bg-gray-900/60 border border-white/10 rounded-3xl p-4 backdrop-blur-md`}>
                <h3 className="text-lg font-bold text-white mb-4 px-2">Kutia Postare</h3>
            ...
            <div className={`${selectedMessage ? 'hidden lg:flex' : 'flex'} flex-col w-full lg:w-1/3 bg-gray-900/60 border border-white/10 rounded-3xl overflow-hidden backdrop-blur-md`}>
                <div className="p-5 border-b border-white/10 bg-white/5"><h3 className="text-lg font-bold text-white flex items-center gap-2"><Mail size={18} className="text-blue-400"/> {activeFolder}</h3></div>
            ...
            <div className={`${selectedMessage ? 'flex' : 'hidden lg:flex'} flex-1 bg-gray-900/60 border border-white/10 rounded-3xl p-6 backdrop-blur-md flex-col relative`}>
                {selectedMessage ? (
                    <>
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar"><div className="bg-black/20 rounded-2xl p-6 border border-white/5"><p className="text-gray-300 leading-relaxed whitespace-pre-wrap">{selectedMessage.content}</p></div></div>
                        <div className="mt-6 pt-4 border-t border-white/10 flex justify-between items-center flex-wrap gap-2">
