...
// PHOENIX PROTOCOL - INBOX V4.0 (UNIFIED ADMIN AESTHETIC)
            ...
            <div className="lg:col-span-2 flex flex-col lg:flex-row gap-6 max-h-[70vh]">
                ...
                <div className={`${selectedMessage ? 'hidden lg:flex' : 'flex'} flex-col w-full lg:w-1/2`}>
                    <Panel className="p-0 overflow-hidden flex flex-col h-full">
                        <div className="p-5 border-b border-border-strong bg-surface flex-shrink-0">
                            ...
                            {loading ? (
                                ...
                                messages.map((msg) => (
                                    <div 
                                        ...
                                        className={`p-4 rounded-xl cursor-pointer transition-all border ${selectedMessage?.id === msg.id ? 'bg-primary/20 border-primary/30' : 'border-transparent hover:bg-hover'}`}
                                    >
                                        <div className="flex justify-between items-start mb-1">
                                            <h4 className="font-bold text-text-primary text-sm truncate">{msg.client_name}</h4>
                                            <span className="text-[10px] text-text-muted">{new Date(msg.created_at).toLocaleDateString()}</span>
                                        </div>
                ...
                <div className={`${selectedMessage ? 'flex' : 'hidden lg:flex'} flex-1`}>
                    <Panel className="p-6 flex flex-col h-full w-full relative">
                        {selectedMessage ? (
                            <>
                                <button onClick={() => setSelectedMessage(null)} className="lg:hidden absolute top-4 left-4 p-2 bg-surface rounded-lg text-text-primary">
                                    <ArrowRight className="rotate-180" size={20}/>
                                </button>
                                <div className="flex items-center gap-4 mb-6 border-b border-border-strong pb-6 mt-8 lg:mt-0 flex-shrink-0">
                                ...
                                <div className="flex-1 overflow-y-auto custom-scrollbar min-h-0">
                                    <div className="bg-surface rounded-2xl p-6 border border-border-strong">
                                        <p className="text-text-secondary leading-relaxed whitespace-pre-wrap">{selectedMessage.content}</p>
                                    </div>
                                </div>
                                <div className="mt-6 pt-4 border-t border-border-strong flex justify-start items-center flex-wrap gap-2 flex-shrink-0">
                                    {activeFolder !== 'ARCHIVED' && (
                                        <button onClick={() => handleAction(selectedMessage.id, 'ARCHIVED')} 
                                            className="flex items-center gap-2 px-4 py-2 bg-surface hover:bg-hover text-text-secondary hover:text-text-primary rounded-lg text-xs font-medium border border-border-strong transition-all">
                                    ...
                                    {activeFolder === 'TRASHED' ? (
                                        <button onClick={() => handleDeletePermanent(selectedMessage.id)} 
                                            className="flex items-center gap-2 px-4 py-2 bg-danger/10 hover:bg-danger/20 text-danger rounded-lg text-xs font-medium border border-danger/20 transition-all">
                                        ...
                                        <button onClick={() => handleAction(selectedMessage.id, 'TRASHED')} 
                                            className="flex items-center gap-2 px-4 py-2 bg-surface hover:bg-danger/10 text-text-secondary hover:text-danger rounded-lg text-xs font-medium border border-border-strong hover:border-danger/20 transition-all">
