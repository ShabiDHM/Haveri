...
// PHOENIX PROTOCOL - PROFIT MODULE V6.7 (TYPE PATH CORRECTION)
// 1. FIX: Changed import path for AI types to 'src/data/types' to resolve TS2459.
// 2. STATUS: Fully synchronized with project type definitions.
...
import { RestockPrediction, SalesTrendAnalysis } from '../../../data/types'; // PHOENIX: Corrected import path
...
export const ProfitModule: React.FC<ProfitModuleProps> = ({ data }) => {
    ...
    return (
        <>
            <div className="bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-md h-auto lg:h-[540px] flex flex-col shadow-xl">
                        ...
                        {lowStockItems.length === 0 ? (
                            ...
                            lowStockItems.map((item: any, idx: number) => (
                                <div 
                                    ...
                                    className="flex justify-between items-center p-3 rounded-xl bg-white/5 border border-white/5 shrink-0 cursor-pointer hover:bg-white/10 hover:border-white/20 transition-all group"
                                >
                                    ...
                                    <span className="text-xs font-mono text-rose-400 flex items-center gap-1 bg-rose-500/10 px-2 py-1 rounded-lg border border-rose-500/20">
                            ...
                            <div className="p-6 border-b border-white/10 bg-purple-900/20 flex justify-between items-start">
                                <div>
                                    <h3 className="text-xl font-bold text-white flex items-center gap-2">
                                        {selectedItem.name}
                                        <span className="text-xs bg-rose-500 text-white px-2 py-0.5 rounded-full">{t('inventory.analysis.critical', 'Kritike')}</span>
                                    </h3>
                            ...
                            <div className="p-6 min-h-[300px]">
                                {loading ? (
                                    <div className="flex flex-col items-center justify-center h-full py-10 space-y-4 text-center">
                                        ...
                                        <p className="text-gray-400 animate-pulse font-medium">{t('inventory.analysis.analyzing', 'Inteligjenca Artificiale po analizon...')}</p>
                                    </div>
                                ) : (
                                    <div className="space-y-6">
                                        <div className="bg-black/40 border border-white/10 rounded-xl p-4 relative overflow-hidden">
                                            ...
                                            <button 
                                                ...
                                                className="w-full mt-3 py-2.5 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-lg font-bold text-sm shadow-lg shadow-blue-600/20 transition-all flex items-center justify-center gap-2 transform hover:scale-[1.02]"
                                            >
                                        ...
                                        <div className="bg-black/40 border border-white/10 rounded-xl p-4 relative overflow-hidden">
                                            ...
                                            <div className="space-y-3">
                                                <div>
                                                    <p className="text-[10px] text-gray-500 uppercase font-black mb-1 tracking-widest">{t('inventory.analysis.performance', 'Performance')}</p>
                                                    <p className="text-sm text-gray-300 leading-relaxed">
                                                        {aiData.trend?.trend_analysis || t('inventory.analysis.unavailable', 'Nuk ka të dhëna mjaftueshme shitjeje.')}
                                                    </p>
                                                </div>
                                                <div className="pt-2 border-t border-white/5">
                                                    <p className="text-[10px] text-gray-500 uppercase font-black mb-1 tracking-widest">{t('inventory.analysis.crossSell', 'Mundësi Cross-Sell')}</p>
                                                    <p className="text-sm text-gray-300">
                                                        {aiData.trend?.cross_sell_opportunities || 'N/A'}
                                                    </p>
                                                </div>
                                            </div>
                                        </div>
                            ...
                            <div className="p-6 space-y-4">
                                <div className="space-y-1">
                                    ...
                                    <textarea 
                                        ...
                                        className="w-full bg-black/40 border border-white/10 rounded-lg p-3 text-white h-24 resize-none focus:border-blue-500/50 outline-none transition-all"
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="space-y-1">
                                        ...
                                        <input type="number" value={poQuantity} onChange={(e) => setPoQuantity(parseFloat(e.target.value) || 0)} className="w-full bg-black/40 border border-white/10 rounded-lg p-3 text-white focus:border-blue-500/50 outline-none transition-all"/>
                                    </div>
                                    <div className="space-y-1">
                                        ...
                                        <input 
                                            ...
                                            className="w-full bg-black/40 border border-white/10 rounded-lg p-3 text-white focus:border-blue-500/50 outline-none transition-all"
                                        />
                                    </div>
                                </div>
                            </div>
                            <div className="p-6 bg-black/20 flex justify-end gap-3">
                                <button onClick={() => setShowPOModal(false)} className="px-5 py-2.5 rounded-xl bg-white/5 text-gray-300 hover:bg-white/10 transition-colors font-medium">
                                    {t('general.cancel')}
                                </button>
                                <button 
                                    onClick={handleConfirmAndGeneratePO} 
                                    disabled={drafting} 
                                    className="px-6 py-2.5 rounded-xl bg-blue-600 text-white shadow-lg hover:bg-blue-500 transition-colors font-bold flex items-center gap-2 disabled:opacity-50"
                                >
