            ...
            <AnimatePresence>
                {showAudit && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-md">
                        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-glass backdrop-blur-xl border border-border-strong rounded-2xl w-full max-w-lg shadow-xl overflow-hidden relative">
                            ...
                            <div className="p-6">
                                {auditLoading ? (
                                    ...
                                    <div className="space-y-6">
                                        <div className="flex items-center gap-4 p-4 bg-surface rounded-xl border border-border-strong">
                                        ...
                                        <div className="bg-surface rounded-xl p-4 border border-border-strong max-h-60 overflow-y-auto custom-scrollbar">
                                            {auditResult?.anomalies.map((note, idx) => (<div key={idx} className="flex gap-3 mb-3 last:mb-0 text-sm text-text-secondary p-2 border-b border-border-strong last:border-0"><div className="min-w-[6px] w-[6px] h-[6px] rounded-full bg-warning-start mt-1.5" /><p>{note}</p></div>))}
                                        ...
                                        <div className="flex gap-3 pt-2">
                                            <button onClick={() => setShowAudit(false)} className="flex-1 py-3 rounded-xl bg-surface hover:bg-hover text-text-secondary font-medium transition-colors border border-border-strong">Rishiko sërish</button>
