            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                <div className="lg:col-span-4 space-y-6">
                    <Panel className="p-6 sm:p-8 flex flex-col items-center text-center border border-border-main bg-surface/30 backdrop-blur-sm shadow-sm">
                        ...
                        <h2 className="text-lg font-bold text-text-primary">{profile?.firm_name || "Kompania"}</h2>
                    </Panel>
                    ...
                    {user?.organization_role === 'OWNER' && (
                        <Panel className="p-6 border border-border-main bg-surface/30 backdrop-blur-sm shadow-sm">
                            ...
                            <form onSubmit={handleInviteUser} className="mb-6">
                                <input type="email" value={inviteEmail} onChange={(e) => setInviteEmail(e.target.value)} disabled={isPlanFull} className="glass-input w-full mb-3 border border-border-main bg-surface/30 backdrop-blur-sm text-text-primary placeholder:text-text-muted" placeholder="email@ekipi.com" />
                                <button type="submit" disabled={inviting} className="w-full py-2 btn-primary rounded-xl font-bold text-xs flex items-center justify-center gap-2 hover-lift shadow-sm">
                            ...
                            <div className="space-y-2 max-h-[200px] overflow-y-auto">
                                {teamMembers.map(member => (
                                    <div key={member.id} className="flex items-center justify-between p-2.5 bg-surface/50 backdrop-blur-sm rounded-xl border border-border-main">
                                        ...
                                        <button onClick={() => handleRemoveMember(member.id)} className="p-1.5 rounded-md text-text-muted hover:text-danger-start hover:bg-danger-start/10 transition-colors hover-lift">
                                            <Trash2 size={14} />
                                        </button>
                                    </div>
                                ))}
                    ...
                    <form onSubmit={handleProfileSubmit}>
                        <Panel className="p-6 sm:p-10 border border-border-main bg-surface/30 backdrop-blur-sm shadow-sm">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div className="md:col-span-2 mt-6"><button type="submit" className="btn-primary w-full py-4 flex items-center justify-center gap-2 rounded-xl hover-lift shadow-sm" disabled={saving}>{saving ? <Loader2 className="animate-spin" /> : <><Save size={18} />RUHAJ</>}</button></div>
                            </div>
                        </Panel>
                    </form>
                </div>
