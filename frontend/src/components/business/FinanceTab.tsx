...
import { 
    ...
    FileSpreadsheet, PiggyBank, ShoppingCart
} from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { apiService } from '../../services/api';
import { 
    Invoice, InvoiceItem, Case, Document, 
    Expense, ExpenseCreateRequest, AnalyticsDashboardData, TopProductItem, PosTransaction
} from '../../data/types';
...
export const FinanceTab: React.FC = () => {
    ...
    const handleViewExpense = async (expense: Expense) => { 
        ...
        try { 
            ...
            if (expense.receipt_url) {
                ...
                url = window.URL.createObjectURL(file); file_name = file.name; mime_type = 'text/plain';
            }
            setViewingUrl(url); 
            setViewingDoc({ id: expense.id, file_name, mime_type, status: 'READY' } as any); 
        } catch { alert(t('error.receiptNotFound', 'Gabim gjatë hapjes.')); } finally { setOpeningDocId(null); } 
    };
    ...
    const handleDownloadExpense = async (expense: Expense) => { 
        try { 
            ...
            if (expense.receipt_url) {
                const { blob, filename: fn } = await apiService.getExpenseReceiptBlob(expense.id); url = window.URL.createObjectURL(blob); filename = fn;
            } else {
    ...
    const submitArchiveExpense = async () => { 
        ...
        try { 
            ...
            if (ex.receipt_url) {
                const { blob, filename } = await apiService.getExpenseReceiptBlob(ex.id); fileToUpload = new File([blob], filename, { type: blob.type }); 
            ...
            alert(t('general.saveSuccess')); setShowArchiveExpenseModal(false); setSelectedCaseForInvoice(""); 
        } catch { alert(t('error.generic')); } 
    };
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500 lg:h-[600px]">
                <div className="lg:col-span-1 flex flex-col gap-6 h-full">
                    ...
                    <div className="bg-background-dark/50 border border-glass-edge rounded-3xl p-6 space-y-3 flex-1 flex flex-col justify-start">
                        ...
                        <QuickActionButton icon={<FileSpreadsheet size={18} />} label={t('finance.import.title', 'Importo Transaksionet')} onClick={() => setShowImportModal(true)} color="text-blue-400" />
                                        ...
                                        <div key={`${tx.type}-${tx.id}`} className="bg-white/5 border border-white/10 rounded-xl p-3 hover:bg-white/10 transition-colors">
                                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                                                <div className="flex items-center gap-3 w-full sm:w-auto overflow-hidden">
                                                    <div className={`p-2 rounded-lg flex-shrink-0 ${tx.type === 'invoice' || tx.type === 'pos' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-rose-500/10 text-rose-400'}`}>
                                                        {tx.type === 'invoice' ? <FileText size={18} /> : tx.type === 'expense' ? getCategoryIcon(tx.label) : <ShoppingCart size={18} />}
                                                    </div>
                                                    <div className="min-w-0 flex-1">
                                                        <h4 className="font-bold text-white text-sm truncate">{tx.label}</h4>
                                                        <p className="hidden sm:block text-xs text-gray-400 font-mono mt-0.5">
                                                            {tx.type === 'invoice' ? `#${(tx as Invoice).invoice_number}` : new Date(tx.date).toLocaleDateString()}
                                                            {tx.type === 'pos' && ` - ${(tx as PosTransaction).payment_method}`}
                                                        </p>
                                                    ...
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        { tx.type !== 'pos' &&
                                                            <div className="flex items-center gap-1">
                                                                <button onClick={() => tx.type === 'invoice' ? handleEditInvoice(tx as Invoice) : handleEditExpense(tx as Expense)} className="p-1.5 hover:bg-white/20 rounded-md text-amber-400 transition-colors" title={t('general.edit')}><Edit2 size={16} /></button>
                                                                <button onClick={() => tx.type === 'invoice' ? handleViewInvoice(tx as Invoice) : handleViewExpense(tx as Expense)} disabled={(tx.type === 'expense' && !(tx as Expense).receipt_url) && openingDocId !== tx.id && !openingDocId} className="p-1.5 hover:bg-white/20 rounded-md text-blue-400 transition-colors disabled:opacity-50" title={t('general.view')}> {openingDocId === tx.id ? <Loader2 size={16} className="animate-spin"/> : <Eye size={16} />}</button>
                                                                <button onClick={() => tx.type === 'invoice' ? downloadInvoice(tx.id) : handleDownloadExpense(tx as Expense)} className="p-1.5 hover:bg-white/20 rounded-md text-green-400 transition-colors" title={t('general.download')}><Download size={16} /></button>
                                                                <button onClick={() => tx.type === 'invoice' ? handleArchiveInvoiceClick(tx.id) : handleArchiveExpenseClick(tx.id)} className="p-1.5 hover:bg-white/20 rounded-md text-indigo-400 transition-colors" title={t('general.archive')}><Archive size={16} /></button>
                                                                <button onClick={() => tx.type === 'invoice' ? deleteInvoice(tx.id) : deleteExpense(tx.id)} className="p-1.5 hover:bg-white/20 rounded-md text-red-400 transition-colors" title={t('general.delete')}><Trash2 size={16} /></button>
                                                            </div>
                                                        }
                                                    </div>
                                                </div>
                                    ...
                                    {filteredHistory.length === 0 ? (
                                        ...
                                        filteredHistory.map((item) => (
                                            <div key={item.id} className="bg-white/5 border border-white/10 rounded-xl overflow-hidden">
                                                <div className="p-4 flex items-center justify-between cursor-pointer hover:bg-white/5 transition-colors" onClick={() => setExpandedCaseId(expandedCaseId === item.id ? null : item.id)}>
                                                    <div className="flex items-center gap-3">
                                                ...
                                                {expandedCaseId === item.id && (
                                                    <div className="bg-black/20 p-4 border-t border-white/5 space-y-2">
                                                        ...
                                                        {item.activity.map((act, idx) => (<div key={`${act.type}-${(act as any).id || idx}`} className="flex justify-between items-center text-sm py-1 border-b border-white/5 last:border-0"><div className="flex items-center gap-3"><span className="text-gray-400 text-xs font-mono">{new Date(act.date).toLocaleDateString('sq-AL')}</span><div className="flex flex-col"><span className="text-white font-medium">{act.label || act.type}</span>
