...
// PHOENIX PROTOCOL - FINANCE TAB V13.3 (FINAL CLEANUP)
// 1. FIX: Removed unused 'TopProductItem' import.
// 2. FIX: Restored full logic for 'generateDigitalReceipt' to use the 'expense' variable.
// 3. STATUS: Zero warnings, full functionality, modern UI.
...
export const FinanceTab: React.FC = () => {
    ...
    // UPDATED HELPER: Uses ArrowUpRight as fallback
    ...
    // PHOENIX FIX: Restored Logic to use 'expense' variable
    const generateDigitalReceipt = (expense: Expense): File => {
        const content = `DËSHMI DIGJITALE E SHPENZIMIT (HAVERI AI)\n------------------------------------------------\n` +
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
            ...
            <div className="flex flex-wrap items-center gap-3 bg-white/5 p-3 rounded-2xl border border-white/5">
                ...
                <ActionButton icon={<FileSpreadsheet size={16} />} label={t('finance.import.title', 'Import')} onClick={() => setShowImportModal(true)} />
                ...
                <div className="flex-1 overflow-hidden relative">
                                        ...
                                        <div className="flex items-center justify-between sm:justify-end gap-6 w-full sm:w-auto">
                                            ...
                                            {tx.type !== 'pos' && (
                                                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                    <button onClick={() => tx.type === 'invoice' ? handleEditInvoice(tx as Invoice) : handleEditExpense(tx as Expense)} className="p-2 hover:bg-white/10 rounded-lg text-amber-400 hover:text-amber-300" title="Edit"><Edit2 size={16} /></button>
                                                    
                                                    {/* RESTORED: Eye with Loading State */}
                                                    <button onClick={() => tx.type === 'invoice' ? handleViewInvoice(tx as Invoice) : handleViewExpense(tx as Expense)} disabled={openingDocId === tx.id} className="p-2 hover:bg-white/10 rounded-lg text-blue-400 hover:text-blue-300" title="View">
                                                        {openingDocId === tx.id ? <Loader2 size={16} className="animate-spin"/> : <Eye size={16} />}
                                                    </button>
                                                    
                                                    {/* RESTORED: Download Button */}
                                                    <button onClick={() => tx.type === 'invoice' ? downloadInvoice(tx.id) : handleDownloadExpense(tx as Expense)} className="p-2 hover:bg-white/10 rounded-lg text-green-400 hover:text-green-300" title="Download">
                                                        <Download size={16} />
                                                    </button>

                                                    {/* RESTORED: Archive Button */}
                                                    <button onClick={() => tx.type === 'invoice' ? handleArchiveInvoiceClick(tx.id) : handleArchiveExpenseClick(tx.id)} className="p-2 hover:bg-white/10 rounded-lg text-indigo-400 hover:text-indigo-300" title="Archive">
                                                        <Archive size={16} />
                                                    </button>

                                                    {/* RESTORED: Delete Button */}
                                                    <button onClick={() => tx.type === 'invoice' ? deleteInvoice(tx.id) : deleteExpense(tx.id)} className="p-2 hover:bg-white/10 rounded-lg text-red-400 hover:text-red-300" title="Delete">
                                                        <Trash2 size={16} />
                                                    </button>
                                                </div>
                                            )}
                    ...
                    {activeTab === 'reports' && (
                        <div className="h-full overflow-y-auto custom-finance-scroll pr-2">
                            {/* Graphs Logic */}
                            {!analyticsData ? <div className="text-center text-gray-500 py-10">No Data</div> : (
            ...
            {/* MODALS (Included for completeness) */}
