        ...
        <motion.div whileHover={{ scale: 1.01 }} onClick={onClick} className="group relative flex flex-col justify-between h-full min-h-[14rem] p-6 rounded-2xl bg-surface/60 border border-border-main hover:border-primary-start/30 transition-all cursor-pointer"> 
            ...
            <div className="pt-4 border-t border-border-main flex justify-between items-center mt-4"> 
                ...
                <div className="flex gap-1 items-center">
                    ...
                    {onShare && <button onClick={(e) => { e.stopPropagation(); onShare(); }} className={`p-2 ${isShared ? 'text-success-start' : 'text-text-muted hover:text-text-primary'}`}><Share2 size={16} /></button>}
                    {onRename && <button onClick={(e) => { e.stopPropagation(); onRename(); }} className="p-2 text-text-muted hover:text-text-primary transition-colors" title={t('general.edit')}><Pencil size={16}/></button>}
                    {!isFolder && (
                        <>
                            <button onClick={(e) => { e.stopPropagation(); onClick(); }} className="p-2 text-text-muted hover:text-primary transition-colors">{isLoading ? <Loader2 className="animate-spin" size={16} /> : <Eye size={16} />}</button>
