        ...
        <div onClick={onClick} className={`group relative flex flex-col justify-between h-full min-h-[14rem] p-6 rounded-2xl transition-all duration-300 cursor-pointer bg-gray-900/40 backdrop-blur-md border border-white/5 shadow-xl hover:shadow-2xl hover:bg-gray-800/60 hover:-translate-y-1 hover:scale-[1.01]`}>
            ...
            <div>
                ...
                <div className="flex flex-col mb-6 relative z-10">
                    <div className="flex items-center gap-2 mb-3 pb-2 border-b border-white/5"><Info className="w-3.5 h-3.5 text-indigo-400" /><span className="text-sm font-bold text-gray-300 uppercase tracking-wider">{isFolder ? t('archive.contents') : t('archive.details')}</span></div>
            ...
            <div className="relative z-10 pt-4 border-t border-white/5 flex items-center justify-between min-h-[3rem]">
                <span className="text-sm font-medium text-indigo-400 group-hover:text-indigo-300 transition-colors flex items-center gap-1">{isFolder ? t('archive.openFolder') : ''}</span>
                <div className="flex gap-1 items-center opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                    {isFolder && type === "Dosje Projekti" && onCaseShare && (<button onClick={(e) => { e.stopPropagation(); onCaseShare(); }} className={`p-2 rounded-lg transition-colors ${isShared ? 'bg-green-500/20 text-green-400 hover:bg-green-500/30' : 'text-gray-600 hover:text-white hover:bg-white/10'}`} title={t('archive.shareCaseTitle')}><Share2 className="h-4 w-4" /></button>)}
                    {!isFolder && onShare && (<button onClick={(e) => { e.stopPropagation(); onShare(); }} className={`p-2 rounded-lg transition-colors ${isShared ? 'bg-green-500/20 text-green-400 hover:bg-green-500/30' : 'text-gray-600 hover:text-white hover:bg-white/10'}`} title={isShared ? t('archive.unshare') : t('archive.share')}><Share2 className="h-4 w-4" /></button>)}
                    {onRename && (<button onClick={(e) => { e.stopPropagation(); onRename(); }} className="p-2 rounded-lg text-gray-600 hover:text-white hover:bg-white/10 transition-colors" title={t('general.edit')}><Pencil className="h-4 w-4" /></button>)}
                    {!isFolder && <button onClick={(e) => { e.stopPropagation(); onClick(); }} className="p-2 rounded-lg text-gray-600 hover:text-blue-400 hover:bg-blue-400/10 transition-colors" title={t('general.view')}>{isLoading ? <Loader2 className="h-4 w-4 animate-spin text-blue-400" /> : <Eye className="h-4 w-4" />}</button>}
                    {!isFolder && onDownload && <button onClick={(e) => { e.stopPropagation(); onDownload(); }} className="p-2 rounded-lg text-gray-600 hover:text-green-400 hover:bg-green-400/10 transition-colors" title={t('general.download')}><Download className="h-4 w-4" /></button>}
                    {onDelete && <button onClick={(e) => { e.stopPropagation(); onDelete(); }} className="p-2 rounded-lg text-gray-600 hover:text-red-400 hover:bg-red-400/10 transition-colors" title={t('general.delete')}><Trash2 className="h-4 w-4" /></button>}
                </div>
            </div>
        </div>
    );
...
export const ArchiveTab: React.FC = () => {
    ...
    const [shareModalCase, setShareModalCase] = useState<Case | null>(null);
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-8">
            ...
            {shareModalCase && (
                <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-background-dark border border-glass-edge rounded-3xl w-full max-w-md p-8 shadow-2xl">
                        <div className="flex justify-between items-center mb-4"><h3 className="text-xl font-bold text-white">{t('archive.shareCaseTitle')}</h3><button onClick={() => setShareModalCase(null)} className="text-gray-500 hover:text-white"><X size={24}/></button></div>
                        <p className="text-sm text-gray-400 mb-6">{t('archive.shareCaseDesc')}</p>
                        <div className={`rounded-xl p-4 flex items-center justify-between cursor-pointer transition-all duration-300 mb-6 ${shareModalCase.is_shared ? 'bg-indigo-500/10 border border-indigo-500/20' : 'bg-black/20 border border-white/5'}`} onClick={handleToggleCaseShare}>
                            <div className="flex items-center gap-3">
                                <div className={`p-2 rounded-lg ${shareModalCase.is_shared ? 'bg-indigo-500 text-white' : 'bg-white/10 text-gray-400'}`}>{shareModalCase.is_shared ? <Eye size={18} /> : <EyeOff size={18} />}</div>
                                <div><h4 className={`text-sm font-bold ${shareModalCase.is_shared ? 'text-indigo-200' : 'text-gray-400'}`}>{shareModalCase.is_shared ? t('archive.sharingEnabled') : t('archive.sharingDisabled')}</h4></div>
                            </div>
                            <div className={`w-10 h-5 rounded-full relative transition-colors ${shareModalCase.is_shared ? 'bg-indigo-500' : 'bg-gray-700'}`}><div className={`absolute top-1 left-1 w-3 h-3 bg-white rounded-full transition-transform ${shareModalCase.is_shared ? 'translate-x-5' : 'translate-x-0'}`} /></div>
                        </div>
                        {shareModalCase.is_shared && (
