...
// PHOENIX PROTOCOL - LINTER CLEANUP V1.1
// 1. CLEANUP: Removed unused 'format' import from 'date-fns'.

import React, { useState } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { useTranslation } from 'react-i18next';
import { Loader2, Target, AlertTriangle, X, Clock, Handshake, CheckSquare, DollarSign, ShieldAlert, Users } from 'lucide-react';
import { useStrategicBriefing, AgendaItem } from '../../hooks/useStrategicBriefing';
import { parseISO } from 'date-fns'; // PHOENIX: 'format' is no longer needed here
...
const BriefingEventModal: React.FC<BriefingEventModalProps> = ({ item, onClose }) => {
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-[100]">
            ...
            >
                ...
                <div className="grid grid-cols-2 gap-4 mt-8">
                    ...
                    <button className="w-full py-3 rounded-xl bg-red-500/10 hover:bg-red-500/20 text-red-400 font-medium transition-colors">{t('calendar.detailModal.delete', 'Fshij')}</button>
                </div>
            </motion.div>
        </div>
    );
