...
export interface UpdateUserRequest { username?: string; email?: string; role?: string; subscription_status?: string; status?: 'active' | 'inactive'; plan_tier?: string; subscription_expiry_date?: string; }
export interface InviteUserRequest { email: string; role: 'MEMBER' | 'VIEWER' | 'ADMIN'; }
export interface CreateWorkspaceRequest { workspace_number: string; title: string; workspace_name?: string; description?: string; clientName?: string; clientEmail?: string; clientPhone?: string; status?: string; }
export interface DeletedDocumentResponse { documentId: string; deletedFindingIds: string[]; }
export interface CalendarEventCreateRequest { title: string; description?: string; start_date: string; end_date?: string; is_all_day?: boolean; event_type: CalendarEvent['event_type']; workspace_id?: string; location?: string; notes?: string; priority?: EventPriority; attendees?: string[]; is_public?: boolean; }
export interface GraphNode { id: string | number; label: string; group: 'Client' | 'Invoice' | 'Expense' | 'Document' | 'Workspace' | 'Default' | 'Inventory' | 'Product'; value?: number; currency?: string; status?: 'Active' | 'Paid' | 'Unpaid' | 'Overdue' | 'Draft' | 'Pending'; color?: string; icon?: string; subLabel?: string; meta?: Record<string, any>; x?: number; y?: number; fx?: number; fy?: number; }
