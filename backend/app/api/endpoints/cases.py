...
# PHOENIX PROTOCOL - CASES ROUTER V5.1 (SINGLETON WORKSPACE)
# 1. NEW ENDPOINT: Added GET /workspace/primary to find the user's default workspace.
# 2. LOGIC: This provides a stable endpoint for the frontend to get the main workspace ID.
# 3. CLEANUP: Removed the now-redundant POST / endpoint from the public schema.
...
async def get_primary_workspace(current_user: Annotated[UserInDB, Depends(get_current_user)], db: Database = Depends(get_db)):
    """
    Finds and returns the primary (first created) workspace for the user.
    This is the core of the Singleton Workspace model.
    """
    # Sort by creation date to get the first one ever made for this user
    workspace = await asyncio.to_thread(db.cases.find_one, 
        {"owner_id": current_user.id}, 
        sort=[("created_at", 1)]
    )
    if not workspace:
        # This can happen if a user was created before the workspace auto-creation logic was added.
        # We can create one on-the-fly here as a self-healing mechanism.
        ...
        try:
            ...
            default_case = CaseCreate(
                ...
                case_number=f"WS-{str(current_user.id)[-6:]}",
                status="Active",
                clientName=current_user.full_name or current_user.username,
...
@router.post("", response_model=CaseOut, status_code=status.HTTP_201_CREATED, include_in_schema=False) # PHOENIX: Hide from docs
@router.post("/", response_model=CaseOut, status_code=status.HTTP_201_CREATED, include_in_schema=False) # PHOENIX: Hide from docs
async def create_new_case(case_in: CaseCreate, current_user: Annotated[UserInDB, Depends(get_current_user)], db: Database = Depends(get_db)):
    # This endpoint is now for internal/power-user use only.
