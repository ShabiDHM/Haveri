...
# PHOENIX PROTOCOL - INTELLIGENCE ENGINE V5.7 (YEAR & CONTEXT FIX)
# 1. FIXED: Capped 'latest_year' at current year to prevent future-dated test data from breaking analysis.
# 2. FIXED: Improved Resilient Filter to ensure Organization data is prioritized over User data.
...
def _get_latest_activity_year(db: Database, context_id: str) -> int:
    """Finds the most recent year with actual activity, capping at the current year."""
    found_years = []
    ...
    try:
        ...
        logger.warning(f"Year discovery failed: {e}")
    
    # Filter out future years that might be in test data
...
async def generate_kpi_insight(request: KpiInsightRequest, current_user: UserInDB = Depends(get_current_user), db: Database = Depends(get_db)):
    # PHOENIX: Correctly identify the Business Context ID
    ...
    if request.kpi_type == 'income':
        ...
        prod_costs = { 
            _normalize(r.get("product_name", "")): sum(_safe_float(ing.get("quantity_required", 0)) * cost_map.get(str(ing.get("inventory_item_id")), 0) for ing in r.get("ingredients", [])) 
            for r in recipes 
        }
        ...
        for s in sales:
            ...
            unit_cost = 0.0
            if sale_name in prod_costs: 
                unit_cost = prod_costs[sale_name]
            else:
                for recipe_name, cost in prod_costs.items():
                    if recipe_name in sale_name: 
                        unit_cost = cost
                        break
        ...
        if total_cogs > 0:
            ...
            summary = f"Nuk u identifikuan receta të lidhura me shitjet e vitit {analysis_year}."
    ...
    try:
        ...
        if result and isinstance(result, dict):
            return RestockPrediction(
                ...
                reason=str(result.get("reason", "Analizë e kryer.")), 
                estimated_cost=_safe_float(result.get("estimated_cost", 0))
            )
        return RestockPrediction(suggested_quantity=0, reason="S'ka të dhëna.", estimated_cost=0)
    ...
    try:
        ...
        if result and isinstance(result, dict):
            return SalesTrendAnalysis(
                trend_analysis=str(result.get("trend_analysis", "Trend stabil.")), 
...
async def get_proactive_insight(): return GeneralInsightResponse(insight="Sistemi aktiv dhe i monitoruar.", sentiment="neutral")
