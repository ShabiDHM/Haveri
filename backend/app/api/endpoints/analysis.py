...
# PHOENIX PROTOCOL - INTELLIGENCE ENGINE V5.4 (STRICT PYLANCE COMPLIANCE)
# 1. FIXED: Resolved all remaining Pylance "None" attribute errors by extracting AI data into local variables.
# 2. STATUS: 100% Type-Safe, Year-Aware, and Fuzzy-Match enabled.
# 3. INTEGRITY: No logic regressions.
...
def _get_latest_activity_year(db: Database, context_id: str) -> int:
    """Discovers the most recent year with actual data records in the database."""
    found_years = []
    try:
...
async def generate_kpi_insight(request: KpiInsightRequest, current_user: UserInDB = Depends(get_current_user), db: Database = Depends(get_db)):
    ...
    if request.kpi_type == 'income':
        ...
        if total_cogs > 0:
            summary = f"Për vitin {analysis_year}, kostoja e mallrave (COGS) është llogaritur në €{total_cogs:.2f} bazuar në recetat tuaja."
            contributors = [f"Kosto Totale: €{total_cogs:.2f}", f"Analiza: {analysis_year}"]
        else:
            summary = f"Nuk u identifikuan receta të lidhura me transaksionet e vitit {analysis_year}."
...
async def predict_restock(request: PredictionRequest, current_user: UserInDB = Depends(get_current_user), db: Database = Depends(get_db)):
    try:
        ...
        # PHOENIX FIX: Strict local variable extraction to satisfy Pylance
        if result is not None and isinstance(result, dict):
            ai_qty = result.get("suggested_quantity", 0)
            ai_reason = result.get("reason", "Nuk ka të dhëna mjaftueshme.")
            ai_cost = result.get("estimated_cost", 0)
            
            return RestockPrediction(
                suggested_quantity=_safe_float(ai_qty), 
                reason=str(ai_reason), 
                estimated_cost=_safe_float(ai_cost)
            )
...
async def analyze_sales_trend(request: PredictionRequest, current_user: UserInDB = Depends(get_current_user), db: Database = Depends(get_db)):
    try:
        ...
        # PHOENIX FIX: Strict local variable extraction to satisfy Pylance
        if result is not None and isinstance(result, dict):
            ai_trend = result.get("trend_analysis", "S'ka të dhëna mjaftueshme.")
            ai_cross = result.get("cross_sell_opportunities", "N/A")
            
            return SalesTrendAnalysis(
                trend_analysis=str(ai_trend), 
                cross_sell_opportunities=str(ai_cross)
            )
