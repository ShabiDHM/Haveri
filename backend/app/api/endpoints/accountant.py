...
# PHOENIX PROTOCOL - ACCOUNTANT ENDPOINT V1.6 (CONTEXT AWARENESS FIX)
# 1. FIXED: Now identifies organization_id vs user_id context for the Auditor.
# 2. STATUS: Functional and Context-Correct.
...
async def accountant_audit_chat(
    ...
    # PHOENIX: Determine if we should query data for the Org or the individual User
    context_id = str(current_user.organization_id) if current_user.organization_id else str(current_user.id)

    try:
        # Pass the context_id instead of just user_id
