...
# PHOENIX PROTOCOL - WORKSPACE SERVICE V1.7 (CASCADE DELETE)
...
def delete_workspace_by_id(db: Database, ws_id: ObjectId, owner: UserInDB):
    """
    PHOENIX DIRECTIVE: Hard Cascade Delete.
    Totally wipes out all project data across all collections.
    """
    ws_id_str = str(ws_id)
    
    # 1. Wipe Financial Data
    db.invoices.delete_many({"case_id": ws_id_str})
    db.expenses.delete_many({"case_id": ws_id_str})
    db.transactions.delete_many({"case_id": ws_id_str})
    
    # 2. Wipe Inventory Data
    db.inventory.delete_many({"case_id": ws_id_str})
    db.recipes.delete_many({"case_id": ws_id_str})
    
    # 3. Wipe Legal & Documentation Data
    db.documents.delete_many({"case_id": ws_id_str})
    db.archive_items.delete_many({"case_id": ws_id_str})
    
    # 4. Wipe Schedule Data
    db.calendar_events.delete_many({"case_id": ws_id_str})
    
    # 5. Finally, remove the project itself
