...
# PHOENIX PROTOCOL - STRATEGIC INTELLIGENCE ENGINE V22.0 (DATA CONTRACT FIX)
# 1. FIX: The '_compile_tactical_agenda' method now returns the FULL event object ('raw').
# 2. FIX: Added 'kind' and 'isCompleted' fields to match the frontend's UIAgendaItem type.
# 3. EFFECT: This resolves the TypeScript error by providing the data contract the modal expects.
...
class StrategicBriefingService:
    ...
    async def _analyze_staff_performance(self) -> Dict[str, Any]:
        ...
        pipeline = [
            {"$match": {"user_id": self.user_id_obj, "issue_date": {"$gte": today_start}, "status": "PAID"}},
        ...
        total_revenue_today = result[0].get("total_revenue", 0) if result else 0
        total_txs = result[0].get("transaction_count", 0) if result else 0
        status = "fire" if total_txs > 15 else "stable" if total_txs > 5 else "sleep"
        score = min(98, int((total_txs / 30.0) * 100))
        return {
            "efficiencyStatus": status, "efficiencyScore": score, "mvpName": "Live Data",
            "mvpTotal": total_revenue_today, "mvpInsight": {"key": "total_transactions_today", "values": {"count": total_txs}},
    ...
    async def _analyze_market_pulse(self) -> Dict[str, Any]:
        ...
        low_stock_pipeline = [
            {"$match": {"user_id": self.user_id_obj, "$expr": {"$lte": ["$current_stock", "$low_stock_threshold"]}}},
        ...
        for i, item in enumerate(bestsellers):
            signals.append({"id": i + 1, "type": "bestseller", "label": item["_id"], "impact": "high", "message": f"{int(item['total_quantity'])} të shitura sot", "action": "Shiko Produktin"})
        if low_stock_items:
    ...
    async def _compile_tactical_agenda(self) -> List[Dict]:
        ...
        for event in events:
            ...
            # PHOENIX FIX: Construct the full UIAgendaItem object
            agenda_item = {
                "id": str(event.get('_id')),
                "title": event.get('title', 'Pa titull'),
                "time": event_date.strftime("%H:%M"),
                "priority": map_api_priority(event.get('priority')),
                "isCompleted": hours_diff < -1, # If event was more than 1 hour ago
                "kind": 'alert' if is_alert else 'event',
                "raw": event # Send the entire raw event object for the modal
            }
