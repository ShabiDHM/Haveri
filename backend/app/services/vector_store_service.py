...
# PHOENIX PROTOCOL - SAAS VECTOR STORE V26.0 (HAVERI ALIGNMENT)
# 1. OPTIMIZATION: Re-routed database retrieval to use centralized connection pools in app.core.db.
# 2. STATUS: Clean, Isolated & Ready for Production.
...
def _get_db():
    """Retrieves the unified database instance using centralized connection pools."""
...
def delete_document_embeddings(user_id: str, document_id: str):
    try: 
        ...
        logger.error(f"SaaS Embedding Deletion Failed: {e}")
...
def copy_document_embeddings(source_document_id: str, target_document_id: str, target_user_id: str, target_case_id: str):
    try:
        ...
        logger.error(f"SaaS Embedding Copy Failed: {e}")